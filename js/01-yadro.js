...
// ЦЕНЫ, КОТОРЫЕ ПРАВЯТ В «КАЛЬКУЛЯЦИИ»: надбавки за размер пакета (M и L к тарифу
// партнёра) и надбавка за перевес. Лежат в ОТДЕЛЬНОЙ таблице `pricing_settings`,
// а не в `calc_settings`, и это принципиально: `calc_settings` читают только по праву
// «Калькуляция», а размер пакета выбирает менеджер заказов, у которого его нет.
// Положи мы цены туда — у него надбавки молча стали бы нулевыми, и заказы размера M
// уходили бы по цене S. Поэтому читать `pricing_settings` может любой вошедший,
// а править — только по праву «Калькуляция» (см. db/15).
//
// Значения по умолчанию — ровно те, что были зашиты в коде до этой таблицы: если её
// нет или поле пустое, всё считается как раньше, а не по нулям.
const PRICING_DEFAULTS={
  size_mail_m:500, size_mail_l:1500,      // почта: надбавка к тарифу партнёра за M и L
  size_courier_m:2000, size_courier_l:4000, // курьер: то же
  weight_free_kg:2, weight_step_fee:100,  // перевес у почтовых: сверх скольких кг и по сколько
};
function priceNorm(key){
  const p=S.pricing;
  if(p&&p[key]!=null&&p[key]!=='')return parseFloat(p[key])||0;
  return PRICING_DEFAULTS[key];
}
const pricingReady=()=>!!S.pricing; // таблицы нет — панель в «Калькуляции» не показываем
