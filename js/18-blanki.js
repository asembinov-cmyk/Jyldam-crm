...
async function blanksGetTracks(batchId){
  ...
  // Ночью у Казпочты обслуживание базы: 26.09.2026 в 02:08 три попытки подряд вернули
  // ORA-01033 («идёт запуск либо останов ORACLE»). Номера при этом не тратятся — ШПИ не
  // выдан, — но большая партия просто уйдёт в пустую работу. Предупреждаем, не запрещаем:
  // окно у них плавающее, и ночью запрос иногда всё же проходит.
  const hour=new Date().getHours();
  const night=hour<7?'\n\nСейчас ночь. В это время у Казпочты бывает обслуживание базы '
    +'(ORA-01033), и треки могут не выдаться вовсе. Номера при этом не тратятся, но партию '
    +'придётся запускать заново — днём такие запросы обычно проходят.':'';
  if(!confirm(`Запросить у Казпочты ${todo.length} номеров?\n\nКаждый номер тратится из диапазона договора и вернуть его нельзя.${night}`))return;
  blanksBusy=true;
