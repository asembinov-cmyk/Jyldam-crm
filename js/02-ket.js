...
function orderToKet(o){
  ...
  // Отправляем при ЛЮБОМ типе доставки, если трек заполнен. Раньше стояло условие
  // «только почтовые» (по их фразе «когда заказ идёт почтой») — из-за него курьерский
  // заказ с треком уходил вообще без трека, и колонка в KET оставалась пустой.
  // Два имени поля сразу — это проба, а не уверенность:
  //   kz_code — так поле называется в ОТВЕТЕ их API (единственное упоминание в документации);
  //   barcode — так называется колонка в самой CRM у KET (сказал их сотрудник).
  // Среди полей ОТПРАВКИ в документации нет ни того, ни другого. Лишние поля их API
  // молча игнорирует (заказ с kz_code создавался без ошибок), поэтому проба безопасна.
  // Когда станет понятно, какое поле сработало, лишнее убрать.
